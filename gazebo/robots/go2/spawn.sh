...
#   TINYNAV_ROBOT_DIR  override for this directory (defaults to the script's own)
# no set -u before the ROS setup: it references unset vars
# (AMENT_TRACE_SETUP_FILES)

DIR=${TINYNAV_ROBOT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}
...
if ros2 pkg prefix ros_gz_sim >/dev/null 2>&1; then
  ...
  # Offline fallback (image without ros_gz_sim): convert the URDF to SDF with
  # the gz CLI and create the model through the gz service directly -- what
  # ros_gz_sim's create does internally.
  echo "[go2] ros_gz_sim missing; gz sdf + ign service fallback"
  ign sdf -p /tmp/go2_robot.urdf > /tmp/go2_robot.sdf \
    || { echo "[go2] SDF CONVERT FAIL"; kill $RSP; exit 1; }
  ign service -s "/world/$WORLD_NAME/create" \
    --reqtype ignition.msgs.EntityFactory --reptype ignition.msgs.Boolean \
    --timeout 10000 \
    --req "sdf_filename: \"/tmp/go2_robot.sdf\", name: \"go2\", \
pose: {position: {z: $SPAWN_Z}}, allow_renaming: true" \
    || { echo "[go2] SPAWN FAIL (service)"; kill $RSP; exit 1; }
fi
sleep 8
